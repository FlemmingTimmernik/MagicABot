...
namespace ScreenshotHelper
{
    ...
    {
        //TODO Get screenshot from not active window
        public static void CaptureApplication(string procName)
        {
            //ScreenshotHelper.ScreenshotHelper.CaptureApplication("DeadByDaylight-Win64-Shipping");
            //return;
        ...
        public static bool CheckSinglePixel(PixelMatch pm)
        {
            var pixel = GetCroppedScreenShot(pm.pixelPosition, 1, 1).GetPixel(0, 0);
            return CheckPixelColor(pixel, pm.matchColor.R, pm.matchColor.G, pm.matchColor.B);
        }


        public static void SaveFullScreenShot(string Filename)
        {
            var result = GetFullScreenShot();
            if (result != null)
        ...
        public static Bitmap GetCroppedScreenShot(Point start, int width, int height)
        {
            for (int x = 0; x < 1; x++)
            {
                try
